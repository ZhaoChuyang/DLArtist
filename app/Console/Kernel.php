...
class Kernel extends ConsoleKernel
{
    ...
    protected function schedule(Schedule $schedule)
    {
        ...
        $schedule->call(function(){
            Log::info('cleanImages Job Started');
            ...
            Log::info('cleanImages Job Ended');
        })->daily();


        $schedule->call(function(){
            Log::info('cleanRedis Job Started');

            Cache::forget('articlesToAll');
            Cache::forget('max_article_id');

            Log::info('cleanRedis Job Ended');
        })->daily();

        //检查现在的用户数是不是即将超出推荐的csv文件最大记录数，如果是就更新csv
        $schedule->call(function(){
            Log::info('addMaxUserNum Job Started');
            ...
            Log::info('addMaxUserNum Job Ended');
        })->daily();

        //每10分钟更新一下推荐列表
        $schedule->call(function(){

            Log::info('updateRecommendationList Job Started');
            ...
            for($user_id=1; $user_id<=$max_user_id; $user_id++){
                for($cate=1; $cate<=6; $cate++){
                    if(Cache::has('user:'.$user_id.':cate:'.$cate)){
                        ...
                        $thisRecord = ($user_id-1)*6+$cate;
                        $csv[$thisRecord][2]=strval($click_num);
                    }
                }
            ...
            Log::info('updateRecommendationList Job Ended');
        })->hourly();
